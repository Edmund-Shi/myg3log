...
namespace g3_test {
enum WriteMode { kAppend = 0, kTruncate = 1 };
...
bool writeTextToFile(const std::string &filename, const std::string &msg,
                     const WriteMode write_mode, bool push_out = true) {
  if (push_out) {
  ...
  std::ios_base::openmode mode =
      std::ios_base::out; // for clarity: it's really overkill since it's an
                          // ofstream
  (kTruncate == write_mode) ? mode |= std::ios_base::trunc
...
uint64_t mean(const std::vector<uint64_t> &v) {
  uint64_t total = std::accumulate(v.begin(), v.end(),
                                   uint64_t(0)); // '0' is the initial value
  return total / v.size();
}

void measurePeakDuringLogWrites(const std::string &title,
                                std::vector<uint64_t> &result);
inline void measurePeakDuringLogWrites(const std::string &title,
                                       std::vector<uint64_t> &result) {

#if defined(G3LOG_PERFORMANCE)
  std::cout << "G3LOG (" << title << ") WORST_PEAK PERFORMANCE TEST"
            << std::endl;
#elif defined(GOOGLE_GLOG_PERFORMANCE)
  std::cout << "GOOGLE_GLOG (" << title << ") WORST_PEAK PERFORMANCE TEST"
            << std::endl;
#else
  ...
  for (uint64_t count = 0; count < g_iterations; ++count) {
    ...
    LOG(INFO) << title << " iteration #" << count << " " << charptrmsg << strmsg
              << " and a float: " << std::setprecision(6) << pi_f;
    auto stop_time = std::chrono::high_resolution_clock::now();
    uint64_t time_us =
        std::chrono::duration_cast<microsecond>(stop_time - start_time).count();
