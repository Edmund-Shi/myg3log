...
void signalHandler(int signal_number, siginfo_t *info, void *unused_context) {
  ...
  using namespace g3::internal;
  {
    ...
    const auto fatal_reason =
        exitReasonName(g3::internal::FATAL_SIGNAL, signal_number);
...
void installSignalHandler() {
  ...
  action.sa_sigaction =
      &signalHandler; // callback to crashHandler for fatal signals
...
// Redirecting and using signals. In case of fatal signals g3log should log the
// fatal signal and flush the log queue and then "rethrow" the signal to exit
namespace g3 {
// References:
...
//
// memset +  sigemptyset: Maybe unnecessary to do both but there seems to be
// some confusion here
...
/// Generate stackdump. Or in case a stackdump was pre-generated and non-empty
/// just use that one i.e. the latter case is only for Windows and test purposes
std::string stackdump(const char *rawdump) {
  ...
  char **messages = backtrace_symbols(
      dump,
      static_cast<int>(size)); // overwrite sigaction with caller's address
...
void installCrashHandler() { installSignalHandler(); }
} // end namespace g3
