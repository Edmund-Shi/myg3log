...
/** logCapture is a simple struct for capturing log/fatal entries. At
 * destruction the captured message is forwarded to background worker. As a
 * safety precaution: No memory allocated here will be moved into the background
 * worker in case of dynamic loaded library reasons instead the arguments are
 * copied inside of g3log.cpp::saveMessage*/
...
LogCapture::LogCapture(const LEVELS &level, g3::SignalType fatal_signal,
                       const char *dump)
...
LogCapture::LogCapture(const char *file, const int line, const char *function,
                       ...
                       g3::SignalType fatal_signal, const char *dump)
    : _file(file), _line(line), _function(function), _level(level),
      _expression(expression), _fatal_signal(fatal_signal) {
...
void LogCapture::capturef(const char *printf_like_message, ...) {
  ...
  const int nbrcharacters =
      vsnprintf_s(finished_message, finished_message_len, _TRUNCATE,
                  printf_like_message, arglist);
#else
  const int nbrcharacters = vsnprintf(finished_message, finished_message_len,
                                      printf_like_message, arglist);
#endif
  va_end(arglist);
