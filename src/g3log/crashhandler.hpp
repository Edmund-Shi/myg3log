...
/** Install signal handler that catches FATAL C-runtime or OS signals
  ...
  (ANSI), abnormal termination SIGFPE   Floating point exception (ANSI) SIGILL
  ILlegal instruction (ANSI) SIGSEGV  Segmentation violation i.e. illegal memory
  reference SIGTERM  TERMINATION (ANSI)  */
...
/// fatal signals are covered in your thread (even if you don't do a LOG(...)
/// call
void installSignalHandlerForThread();
#else
typedef int SignalType;
/// Probably only needed for unit testing. Resets the signal handling back to
/// default which might be needed in case it was previously overridden The
/// default signals are: SIGABRT, SIGFPE, SIGILL, SIGSEGV, SIGTERM
void restoreSignalHandlerToDefault();

std::string signalToStr(int signal_number);

// restore to whatever signal handler was used before signal handler
// installation
...
///     so users of g3log with zcmq should then use the @ref
///     overrideSetupSignals , likely with the original set of signals but with
///     SIGTERM removed
///
/// call example:
///  g3::overrideSetupSignals({ {SIGABRT, "SIGABRT"}, {SIGFPE,
///  "SIGFPE"},{SIGILL, "SIGILL"},
//                          {SIGSEGV, "SIGSEGV"},});
...
void exitWithDefaultSignalHandler(const LEVELS &level,
                                  g3::SignalType signal_number);
} // namespace internal
} // namespace g3
