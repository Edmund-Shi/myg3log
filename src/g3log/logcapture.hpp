...
/**
 * Simple struct for capturing log/fatal entries. At destruction the captured
 * message is forwarded to background worker. As a safety precaution: No memory
 * allocated here will be moved into the background worker in case of dynamic
 * loaded library reasons
 */
struct LogCapture {
  /// Called from crash handler when a fatal signal has occurred (SIGSEGV etc)
  LogCapture(const LEVELS &level, g3::SignalType fatal_signal,
  ...
  LogCapture(const char *file, const int line, const char *function,
             ...
             g3::SignalType fatal_signal = SIGABRT, const char *dump = nullptr);

  // At destruction the message will be forwarded to the g3log worker.
  // In the case of dynamically (at runtime) loaded libraries, the important
  // thing to know is that all strings are copied, so the original are not
  // destroyed at the receiving end, only the copy
  virtual ~LogCapture();
...
#define G3LOG_FORMAT_STRING
  ...
  capturef(G3LOG_FORMAT_STRING const char *printf_like_message,
           ...); // 2,3 ref:  http://www.codemaestro.com/reviews/18
#endif
