 ...
 * If the sink receives a std::string it will be the std::string toString()...
 * function that will format the data into a string
 *
 * For sinks that receive a LogMessage they can either use the toString()
 * function, or use the helper functions or even the public raw data to format
 * the saved log message any desired way.
 */
struct LogMessage {
  ...
  std::string timestamp(const std::string &time_format = {
                            internal::date_formatted + " " +
                            internal::time_formatted}) const;
  ...
  LogMessage(std::string file, const int line, std::string function,
             const LEVELS level);
  ...
  std::string
  toString(LogDetailsFunc formattingFunc = DefaultLogDetailsToString) const;
