...
// the DEBUG logging level for G3log. In that case they can instead use the
// define
//  "CHANGE_G3LOG_DEBUG_TO_DBUG" and G3log's logging level DEBUG is changed to
//  be DBUG
#if (defined(CHANGE_G3LOG_DEBUG_TO_DBUG))
#if (defined(DBUG))
#error                                                                         \
    "DBUG is already defined elsewhere which clashes with G3Log's log level DBUG"
#endif
#else
#if (defined(DEBUG))
#error                                                                         \
    "DEBUG is already defined elsewhere which clashes with G3Log's log level DEBUG"
#endif
#endif
...
// Levels for logging, made so that it would be easy to change, remove, add
// levels -- KjellKod
struct LEVELS {
  // force internal copy of the const char*. This is a simple safeguard for when
  // g3log is used in a "dynamic, runtime loading of shared libraries"
...
// file the logging level you need then insert it using
// g3::only_change_at_initialization::addLogLevel(...). Please note that this
// only works for dynamic logging levels.
//
// There should be NO reason for modifying this source file when adding custom
// levels
//
// When dynamic loggins levels are disabled then adding your own logging levels
// is not required as the new logging level by default will always be enabled.
//
...
//  LOG(MYFATL) << "this will just work, and it will be counted as a FATAL
//  event";
namespace g3 {
...
/// helper function to tell the logger if a log message was fatal. If it is it
/// will force a shutdown after all log entries are saved to the sinks
