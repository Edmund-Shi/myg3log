...
struct LogWorkerImpl final {
  ...
  std::unique_ptr<kjellkod::Active> _bg; // do not change declaration order. _bg
                                         // must be destroyed before sinks
...
/// addSink( sink, default_call ) which returns a handle to the sink. See below
/// and REAME for usage example save( msg ) : internal use fatal ( fatal_msg ) :
/// internal use
...
public:
  ...
  /// Creates the LogWorker with no sinks. See exampel below on @ref addSink for
  /// how to use it if you want to use the default file logger then see below
  /// for @ref addDefaultLogger
  ...
  A convenience function to add the default g3::FileSink to the log worker
   ...
   std::future<std::string> log_file_name =
  sinkHandle->call(&FileSink::fileName); std::cout << "The filename is: " <<
  log_file_name.get() << std::endl;
   //   something like: /tmp/my_test_log.g3log.20150819-100300.log
   */
  std::unique_ptr<FileSinkHandle>
  addDefaultLogger(const std::string &log_prefix,
  ...
  /// @param call the default call that should receive either a std::string or a
  /// LogMessageMover message
  /// @return handle to the sink for API access. See usage example below at @ref
  /// addDefaultLogger
  template <typename T, typename DefaultLogCall>
  std::unique_ptr<g3::SinkHandle<T>> addSink(std::unique_ptr<T> real_sink,
                                             DefaultLogCall call) {
    using namespace g3;
  ...
  /// internal:
  //  pushes a fatal message on the queue, this is the last message to be
  //  processed
  /// this way it's ensured that all existing entries were flushed before
  /// 'fatal' Will abort the application!
