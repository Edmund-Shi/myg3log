 ...
 *          Helper functionality to put packaged_tasks in standard container.
 * This is especially helpful for background thread processing a la async but
 * through an actor pattern (active object), thread pool or similar. Created:
 * 2012 by Kjell Hedström
 *
 * COMMUNITY THANKS:
 * The code below is in large thanks to exemplifying code snippets from
 * StackOverflow question/answer:
 * http://stackoverflow.com/questions/6230893/developing-c-concurrency-library-with-futures-or-similar-paradigm
 * and a discussion between Lars Gullik Bjønnes and Jonathan Wakely's at:
 * http://gcc.gnu.org/ml/gcc-help/2011-11/msg00052.html
 *
 * Both are highly recommended reads if you are interested in c++ concurrency
 * library
 *  - Kjell, 2012
 *
