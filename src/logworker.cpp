...
void LogWorkerImpl::bgFatal(FatalMessagePtr msgPtr) {
  // this will be the last message. Only the active logworker can receive a
  // FATAL call so it's safe to shutdown logging now
...
LogWorker::~LogWorker() {
  ...
  // However, the waiting below ensures that all messages until this point are
  // taken care of before any internals/LogWorkerImpl of LogWorker starts to be
  // destroyed. i.e. this avoids a race with another thread slipping through the
  // "shutdownLogging" and calling calling ::save or ::fatal through LOG/CHECK
  // with lambda messages and "partly deconstructed LogWorkerImpl"
  //
  //   Any messages put into the queue will be OK due to:
  //  *) If it is before the wait below then they will be executed
  //  *) If it is AFTER the wait below then they will be ignored and NEVER
  //  executed
  auto bg_clear_sink_call = [this] { _impl._sinks.clear(); };
  auto token_cleared = g3::spawn_task(bg_clear_sink_call, _impl._bg.get());
  token_cleared.wait();

  // The background worker WILL be automatically cleared at the exit of the
  // destructor However, the explicitly clearing of the background worker
  // (below) makes sure that there can be no thread that manages to add another
  // sink after the call to clear the sinks above.
  //   i.e. this manages the extremely unlikely case of another thread calling
  // addWrappedSink after the sink clear above. Normally adding of sinks should
  // be done in main.cpp and be closely coupled with the existance of the
  // LogWorker. Sharing this adding of sinks to other threads that do not know
  // the state of LogWorker is considered a bug but it is dealt with nonetheless
  // below.
  //
  // If sinks would already have been added after the sink clear above then this
  // reset will deal with it without risking lambda execution with a partially
  // deconstructed LogWorkerImpl Calling g3::spawn_task on a nullptr Active
  // object will not crash but return a future containing an appropriate
  // exception.
...
void LogWorker::addWrappedSink(
    std::shared_ptr<g3::internal::SinkWrapper> sink) {
