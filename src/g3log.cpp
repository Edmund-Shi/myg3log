 ...
 * and at least in "spirit" influenced from the following sources
 * 1. kjellkod.cc ;)
 * 2. Dr.Dobbs, Petru Marginean:
 * http://drdobbs.com/article/printableArticle.jhtml?articleId=201804215&dept_url=/cpp/
...
g3::LogWorker *g_logger_instance =
    nullptr; // instantiated and OWNED somewhere else (main)
...
const std::function<void(void)> g_pre_fatal_hook_that_does_nothing =
    [] { /*does nothing */ };
...
void initializeLogging(LogWorker *bgworker) {
  std::call_once(g_initialize_flag, [] { installCrashHandler(); });
  ...
  if (internal::isLoggingInitialized() || nullptr == bgworker) {
    ...
    exitMsg << "\t(due to multiple initializations? : " << std::boolalpha
            << internal::isLoggingInitialized();
    exitMsg << ", due to nullptr == bgworker? : " << std::boolalpha
            << (nullptr == bgworker) << ")";
 ...
 *  It will be called just before sending the fatal message, @ref
 * pushFatalmessageToLogger It will be reset to do nothing in
 * ::initializeLogging(...) so please call this function, if you ever need to,
 * after initializeLogging(...)
 */
...
std::function<void(FatalMessagePtr)> g_fatal_to_g3logworker_function_ptr =
    internal::pushFatalMessageToLogger;
...
/**
 * Shutdown the logging by making the pointer to the background logger to
 * nullptr. The object is not deleted that is the responsibility of its owner. *
 */
...
/** explicits copy of all input. This is makes it possibly to use g3log across
 * dynamically loaded libraries i.e. (dlopen + dlsym)  */
void saveMessage(const char *entry, const char *file, int line,
                 const char *function, const LEVELS &level,
                 const char *boolean_expression, int fatal_signal,
                 const char *stack_trace) {
  LEVELS msgLevel{level};
  LogMessagePtr message{
      std::make_unique<LogMessage>(file, line, function, msgLevel)};
  ...
  if (internal::wasFatal(level)) {
    ...
    if (g_fatal_hook_recursive_counter.load() > 1) {
      message.get()
          ->write()
          .append("\n\n\nWARNING\n"
                  "A recursive crash detected. It is likely the hook set with "
                  "'setFatalPreLoggingHook(...)' is responsible\n\n")
          .append("---First crash stacktrace: ")
          .append(first_stack_trace)
          .append("\n---End of first stacktrace\n");
    }
    FatalMessagePtr fatal_message{std::make_unique<FatalMessage>(
        *(message._move_only.get()), fatal_signal)};
    // At destruction, flushes fatal message to g3LogWorker
    // either we will stay here until the background worker has received the
...
/**
 * save the message to the logger. In case of called before the logger is
 * instantiated the first message will be saved. Any following subsequent
 * unitnialized log calls will be ignored.
 *
 * The first initialized log entry will also save the first uninitialized log
 * message, if any
 * @param log_entry to save to logger
 */
void pushMessageToLogger(
    LogMessagePtr incoming) { // todo rename to Push SavedMessage To Worker
 ...
 * will sleep forever (i.e. until the background thread catches up, saves the
 * fatal message and kills the software with the fatal signal.
 */
void pushFatalMessageToLogger(FatalMessagePtr message) {
  if (!isLoggingInitialized()) {
    ...
    internal::exitWithDefaultSignalHandler(message.get()->_level,
                                           message.get()->_signal_id);
  }
