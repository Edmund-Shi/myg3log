...
std::string localtime_formatted_fractions(const g3::system_time_point &ts,
  ...
  for (size_t pos = 0;
       (pos = format_buffer.find(g3::internal::kFractionalIdentier, pos)) !=
       std::string::npos;
    ...
    format_buffer.replace(
        pos, g3::internal::kFractionalIdentier.size() + padding, value);
  }
...
std::string put_time(const struct tm *tmb, const char *c_time_format) {
  ...
  char buffer[size]; // IMPORTANT: check now and then for when gcc will
                     // implement std::put_time.
  //                    ... also ... This is way more buffer space then we need

  auto success = std::strftime(buffer, size, c_time_format, tmb);
  // In DEBUG the assert will trigger a process exit. Once inside the
  // if-statement the 'always true' expression will be displayed as reason for
  // the exit
  //
  // In Production mode
...
std::string localtime_formatted(const g3::system_time_point &ts,
  ...
  return g3::put_time(
      &t, format_buffer.c_str()); // format example: //"%Y/%m/%d %H:%M:%S");
}
} // namespace g3
